import type { TraitManifestEntry } from '../types'
...
export const AboutPanel = ({ acsYear, generatedAt, totalPopulation, traits }: Props) => {
  const modeledSources = Array.from(new Set(traits.map((t) => t.source))).join(', ')
    ...
    <section className="glass gradient-border overflow-hidden rounded-3xl">
      ...
      <div className="border-b border-white/5 bg-dark-700/30 px-6 py-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500">ACS + BRFSS + ATUS (aggregated only)</p>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-5 p-6">
        <p className="text-sm leading-relaxed text-slate-300">
          Estimates are based on U.S. Census Bureau American Community Survey (ACS) {acsYear} 1-year tables
          plus modeled probabilities from BRFSS and ATUS microdata. Everything is precomputed offline and
          displayed as aggregated counts—no individual records are surfaced.
        </p>
        ...
        <div className="space-y-3">
          ...
          <ul className="space-y-2 text-sm text-slate-400">
            {[
              ...
              { label: 'Age bands', value: 'ACS B01001 recoded into adult bands (18-24, 25-34, …, 65+)' },
              { label: 'Children', value: 'DP02 households with own children under 18, scaled to total population' },
              { label: 'Modeled traits', value: modeledSources || 'BRFSS + ATUS' },
            ].map((item) => (
        ...
        <div className="space-y-3">
          ...
          <ul className="space-y-2 text-sm text-slate-400">
            <li className="flex items-start gap-2">
              ...
              <span>ACS cell backbone: sex × region × adult age bands. Filters on those dimensions sum cells directly.</span>
            </li>
            <li className="flex items-start gap-2">
              <span className="mt-1.5 h-1.5 w-1.5 shrink-0 rounded-full bg-glow-500/60" />
              <span>Modeled traits: weighted logistic regression on BRFSS/ATUS, post-stratified to ACS cells. Probabilities are multiplied assuming conditional independence across traits.</span>
            </li>
            <li className="flex items-start gap-2">
              <span className="mt-1.5 h-1.5 w-1.5 shrink-0 rounded-full bg-glow-500/60" />
              <span>Other demographic filters (race, children, income, education) apply ACS shares uniformly across cells.</span>
            </li>
          </ul>
        </div>
        ...
        <div className="flex items-center gap-2 rounded-xl border border-dark-300 bg-dark-700/50 px-4 py-3 text-xs text-slate-500">
          ...
          <span>
            Base population: {totalPopulation.toLocaleString('en-US')} (ACS {acsYear}). Generated at{' '}
        ...
        <div className="flex flex-wrap gap-3 pt-2">
          {[
            ...
            { label: 'BRFSS overview', href: 'https://www.cdc.gov/brfss/' },
            { label: 'ATUS overview', href: 'https://www.bls.gov/tus/' },
          ].map((link) => (
            <a
              key={link.label}
              href={link.href}
